        ...
        <t-card title="策略配置" :bordered="false">
          <t-form :data="form" label-width="120px">
            <t-form-item label="选择股票">
              <t-input v-model="form.stockCode" placeholder="如 600519" />
            </t-form-item>
...
function renderEquity(equity: { date: string; value: number }[], bench: { date: string; value: number }[]) {
  if (!equityRef.value) return;
  if (!chart) chart = echarts.init(equityRef.value);
  const dates = equity.map((c) => c.date.slice(5));
  const benchMap = new Map(bench.map((b) => [b.date.slice(5), b.value]));
  chart.setOption({
    tooltip: { trigger: 'axis' },
    legend: { data: ['策略净值', '基准(买入持有)'] },
    ...
    series: [
      ...
      },
      {
        name: '基准(买入持有)',
        type: 'line',
        data: dates.map((d) => benchMap.get(d) ?? null),
        smooth: true,
