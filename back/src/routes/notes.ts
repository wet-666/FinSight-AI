...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  const { title, content, stockCode } = req.body;
  if (stockCode) {
    await query(
      'UPDATE notes SET title = ?, content = ?, stock_code = ? WHERE id = ? AND user_id = ?',
      [title, JSON.stringify(content), stockCode, req.params.id, req.userId]
    );
  } else {
    await query(
      'UPDATE notes SET title = ?, content = ? WHERE id = ? AND user_id = ?',
      [title, JSON.stringify(content), req.params.id, req.userId]
    );
  }
